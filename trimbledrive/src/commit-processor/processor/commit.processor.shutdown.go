...
func ShutDown(
	commitProcessorConfig *config.CommitProcessorConfig,
	logger *zap.Logger) {

	sigchan := make(chan os.Signal, 1)
	signal.Notify(sigchan, syscall.SIGINT, syscall.SIGTERM)

	reader := shutdownKafkaReader(commitProcessorConfig)
...
func shutdownKafkaReader(config *config.CommitProcessorConfig) *kafka.Reader {
	shutdownKafkaConfig := config.Kafka
	shutdownKafkaConfig.Topic = os.Getenv("GO_CONTROL_TOPIC")
	shutdownKafkaConfig.GroupID = shutdownKafkaConfig.Topic + "-commit-processor-shutdown"
